...
from edisoncar.parts.datastore import Tub
...
class CreateCar(BaseCommand):
    ...
    def create_car(self, path, template='edison1', overwrite=False):
        """
        This script sets up the folder struction for edison to work.
        It must run without edison installed so that people installing with
        docker can build the folder structure for docker to mount to.
        """

        #these are neeeded incase None is passed as path
        path = path or '~/d2'
        template = template or 'edison1'
...
class CalibrateCar(BaseCommand):    
    ...
    def run(self, args):
        from edisoncar.parts.actuator import PCA9685
...
class Sim(BaseCommand):
    '''
    Start a websocket SocketIO server to talk to a edison simulator
    '''
    ...
    def run(self, args):
        ...
        '''
        import socketio
        from edisoncar.parts.simulation import SteeringServer
        from edisoncar.parts.keras import KerasCategorical, KerasLinear
...
class ShowHistogram(BaseCommand):
    ...
    def show_histogram(self, tub_paths, record_name):
        ...
        from edisoncar.parts.datastore import TubGroup
...
class ShowPredictionPlots(BaseCommand):
    ...
    def plot_predictions(cfg, tub_paths, model_path):
        ...
        import pandas as pd
        from edisoncar.parts.datastore import TubGroup
        from edisoncar.parts.keras import KerasCategorical
...
def execute_from_command_line():
    """
    This is the fuction linked to the "edison" terminal command.
    """
    commands = {
            'createcar': CreateCar,
            'findcar': FindCar,
    ...
    if command_text in commands.keys():
        ...
        ec.utils.eprint('Usage: The availible commands are:')
        ec.utils.eprint(list(commands.keys()))
